/**
 * Local Fluent repository provider. Ported from research-agent.
 *
 * WORKER ADAPTATION: `node:fs`/`node:path` are replaced by the Worker shims. A
 * Worker has no local repository, so `existsSync` is false, `readdir` is empty
 * and `readFile` throws — `inspectFluentProject` therefore returns no evidence
 * (truthful in a Worker). The deletion-safety logic and the trace contract are
 * preserved unchanged; `assessDeletionRisk` still routes through the policy.
 */
import {
  existsSync,
  readFile,
...
import { createId, nowIso } from "../core/types";
