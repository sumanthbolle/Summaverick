/**
 * Layer 1 — SDK `explain` provider. Ported from research-agent.
 *
 * WORKER ADAPTATION: `node:fs`, `node:path`, `node:crypto` and `node:child_process`
 * are replaced by the Worker shims in ../runtime/node-shims (see that file). A
 * Worker has no local Fluent project and cannot run the SDK CLI, so
 * `getInstalledVersion` returns null and `isExplainAvailable` returns false —
 * Layer 1 is therefore skipped by the workflow, which is the truthful state in a
 * Worker. All control flow, caching, command-policy checks, version resolution
 * (which uses fetch, and still works), and the emitted trace are preserved.
 */
import {
  createId,
  isVersionAtLeast,
