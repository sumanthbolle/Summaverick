/**
 * Ported from research-agent/src/domains/servicenow/config.ts.
 * Only change: `loadServiceNowDomainConfigFromEnv` now takes a plain
 * `Record<string, string | undefined>` (e.g. the Worker `Env`) instead of
 * `NodeJS.ProcessEnv`, since the Worker has no `process.env` type.
 */
